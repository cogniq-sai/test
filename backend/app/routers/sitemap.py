...
def check_plugins(site: dict = Depends(get_site_from_api_key)):
    """
    ...
    try:
        ...
        if active_plugins:
            from app.services.notification_service import notify_plugin_detected
            notify_plugin_detected(site_id, active_plugins)
...
def generate_sitemap_suggestion(site: dict = Depends(get_site_from_api_key)):
    """
...
def get_sitemap_suggestions(site: dict = Depends(get_site_from_api_key)):
    """
...
def update_sitemap_status(
    ...
    site: dict = Depends(get_site_from_api_key)
):
    """
